    ...
    with torch.no_grad():
        for batch in itertools.islice(test_loader, min(len(test_loader), config.test_batch_limit)):
            ...
            recon_loss = model.module.masked_reconstruction_loss(
                local_embeddings, time_series, attention_mask
            )
...
def main() -> None:
    ...
    # PYTHONPATH=/home2/lijinbo/Projects/AnomalyLlava-master/ python src/models/Moirai/AnomalyLlava_pretrain_multi_full_recon.py
    ...
    # config.ts_config.num_layers = 2
    config.ts_config.patch_size = 16
    config.checkpoint_dir = "experiments/checkpoints/pretrain_single_full_recon"
    config.cuda_devices = "3"
    config.mixed_precision = False
    config.dist_port = "16998"
    config.early_stopping_patience = 7  # Stop training if validation loss doesn't improve for 10 epochs
    config.pretrain_data_path = "/home2/lijinbo/Projects/dataset/ChatTS/single_samples_normal_100_10000"
