...
from .anomalyclip.full_reconstruction import TimeSeriesPretrainModel, create_random_mask, collate_fn
            ...
            for i, (x, mask) in loop:
                ...
                if visualize:
                    self.visualize_reconstruction(original=time_series[0].cpu().numpy(),
                                                ...
                                                save_path=f"/home/lihaoyang/Huawei/TSB-AD/Synthetic/full_mask_anomaly_head_AnomalyCLIP_Reconstruction_5000/plot/{data_index}_reconstruction_visualization.png")

        return scores



    def visualize_reconstruction(self, original, reconstructed, mask, scores, save_path=None):
        ...
        for start, end in mask_regions:
            ax1.axvspan(start, end, alpha=0.2, color='red', label='Masked Region' if start == mask_regions[0][0] else "")
